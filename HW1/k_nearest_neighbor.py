...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        Input / Output: Same as compute_distances_two_loops
        """
        # Computing the squared norms of the test and training points
        test_squared = np.sum(X ** 2, axis=1).reshape(-1, 1)  # Shape (num_test, 1)
        train_squared = np.sum(self.X_train ** 2, axis=1)     # Shape (num_train,)

        # Computing the pairwise inner product between test and train points
        cross_term = np.dot(X, self.X_train.T)  # Shape (num_test, num_train)

        # Combining to get the squared L2 distance and then taking the square root
